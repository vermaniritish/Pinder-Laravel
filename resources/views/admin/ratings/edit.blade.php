						...
						<div class="col-lg-6">
							<div class="form-group">
									...
									<select required class="form-control" name="rating">
										<option {{ old('rating',$page->rating) == '1' ? 'selected' : '' }}
											value="1"> 1</option>
										<option {{ old('rating',$page->rating) == '2' ? 'selected' : '' }}
											value="2"> 2</option>
										<option {{ old('rating',$page->rating) == '3' ? 'selected' : '' }}
											value="3"> 3</option>
										<option {{ old('rating',$page->rating) == '4' ? 'selected' : '' }}
											value="4"> 4</option>
										<option {{ old('rating',$page->rating) == '5' ? 'selected' : '' }}
											value="5"> 5</option>
									</select>
									@error('rating')
										<small class="text-danger">{{ $message }}</small>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									...
									<textarea required rows="2" class="form-control" placeholder="Message" required name="message">{{ old('message',$page->message) }}</textarea>
									@error('message')
										<small class="text-danger">{{ $message }}</small>
									@enderror
								</div>
									...
									<div class="form-group">
										...
										>
							                ...
							                <div class="fixed-edit-section">
							                	@include('admin.partials.previewFileRender', ['file' => $page->image, 'relationType' => 'ratings.image', 'relationId' => $page->id ])
							                </div>
										</div>
									</div>
								</div>
